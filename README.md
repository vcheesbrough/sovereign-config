...
The production Compose stack contains only PostgreSQL and Sovereign Config. It publishes no application, PostgreSQL, metrics, logging, or tracing ports. Traefik must provide the external `proxy-backend` network and is the only supported ingress. The operator must pre-create the external volume named by `POSTGRES_DATA_VOLUME` that holds the PostgreSQL data directory; Compose deliberately refuses to create a default volume, so the volume's storage, backup, and retention are the operator's to choose.

Sovereign Config applies application-level encryption to every secret-classified configuration value. Values are sealed with XChaCha20-Poly1305 under the key supplied through `SOVEREIGN_CONFIG_VALUE_ENCRYPTION_KEY`, and each secret is bound to the row that stores it, so PostgreSQL only ever holds ciphertext: a dump, a backup, a replication stream, or direct SQL access yields nothing readable. Values classified as plain are stored as-is and are not protected this way. Encrypting the volume and the backups remains worthwhile defence in depth, but it is no longer the only thing standing between a stolen disk and your secrets.
